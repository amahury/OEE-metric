...
│   ├─ run_all.sh         # Bash file running the whole pipeline
    └─ metric_validation/
        ├─ bcell_diff.bnet         # Cell Collective Model on B Cell Differentiation 
        ├─ cardi_devel.bnet        # Cell Collective Model on Cardiac Development 
        ├─ tcell_diff.bnet         # Cell Collective Model on T Cell Differentiation
        ├─ eca_benchmark.csv       # Results obtained after running metric_comparison.py
        ├─ grn_benchmark.csv       # Results obtained after running grn_bnet_benchmark.py
        ├─ metric_comparison.csv   # Results obtained after running metric_comparison.py
        ├─ eca_benchmark.py        # Script dedicated to test Ω on Elementary Cellular Automata
        ├─ grn_bnet_benchmark.py   # Script dedicated to test Ω on curated GRN Boolean models 
        ├─ metric_comparison.py    # Script dedicated to test Ω with respect other novelty indexes
        ├─ plot_eca.py             # It takes eca_benchmark.csv as input 
        ├─ plot_grn_benchmark.py   # It takes grn_benchmark.csv as input 
        └─ plot_metric_scatter.py  # It takes metric_comparison.csv as input 
├─ notebooks/
│   ├─ plots_homo.zip     # Jupyter Notebooks containing the homogeneous RBN analyses & plots
│   └─ plots_hetero.zip   # Jupyter Notebooks containing the heterogeneous RBN analyses & plots
├─ data/
...
│   └─ SM_OEEMetric_v2    # Supplementary Material of the paper
